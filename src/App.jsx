...
import { Search, Plus, X, Users, Wallet, Ban, LogOut, Loader2, Lock, Mail, Pencil, History, ShieldCheck, Upload, Trash2, CheckCircle2, ChevronDown, ChevronUp, MapPin, Navigation, Phone, MessageCircle, FileText } from "lucide-react";
...
// Hitung monthKey untuk bulan berikutnya dari sebuah monthKey (dipakai untuk fitur "bayar dimuka").
const nextMonthKey = (key) => {
  const [y, m] = key.split("-").map(Number);
  return monthKey(new Date(y, m, 1)); // m (bukan m-1) karena Date bulan 0-index -> otomatis maju 1 bulan
};
...
// Buka jendela baru berisi invoice yang sudah diformat, lalu langsung munculkan dialog print browser
// (dari situ user tinggal pilih "Simpan sebagai PDF" untuk mengunduh, atau cetak langsung ke printer).
// payment.status yang termasuk isLunasStatus() -> dicetak sebagai "Bukti Pembayaran" (sudah lunas).
// payment.status lain (mis. "tagihan") -> dicetak sebagai "Tagihan" (belum bayar, jumlah yang harus dibayar).
function printInvoice(customer, payment, penagihNama) {
  const isPaidDoc = isLunasStatus(payment.status);
  const noInvoice = `${(payment.month || "").replace("-", "")}-${String(customer.id || "").slice(-6).toUpperCase()}`;
  const docLabel = isPaidDoc ? "Bukti Pembayaran" : "Tagihan Pembayaran";
  const statusLabel = isPaidDoc ? (STATUS_LABEL[payment.status]?.label || payment.status) : "Belum Dibayar";
  const amountLabel = isPaidDoc ? "Jumlah Dibayar" : "Jumlah Tagihan";
  const dateLabel = isPaidDoc ? "Tanggal Bayar" : "Tanggal Tagihan";
  const pillBg = isPaidDoc ? "#E6F4F1" : "#FBEAE6";
  const pillColor = isPaidDoc ? "#2A9D8F" : "#B0362A";
  const w = window.open("", "_blank", "width=480,height=700");
  if (!w) { alert("Popup diblokir browser. Izinkan popup untuk situs ini agar invoice bisa dibuka."); return; }
  w.document.write(`
    <html><head><title>${docLabel} ${noInvoice}</title>
    <meta charset="utf-8" />
    <style>
      * { box-sizing: border-box; }
      body { font-family: -apple-system, Arial, sans-serif; color: #1A1D23; margin: 0; padding: 28px; }
      .head { display:flex; justify-content:space-between; align-items:flex-start; border-bottom: 2px solid #16324F; padding-bottom: 14px; margin-bottom: 18px; }
      .brand { font-size: 18px; font-weight: 700; color: #16324F; }
      .sub { font-size: 11px; color: #9CA3AF; margin-top: 2px; }
      .inv-title { text-align:right; }
      .inv-title .t { font-size: 13px; letter-spacing: 1px; color: #9CA3AF; }
      .inv-title .n { font-size: 15px; font-weight: 700; color: #16324F; }
      table { width: 100%; border-collapse: collapse; margin: 14px 0; }
      td { padding: 6px 0; font-size: 13px; vertical-align: top; }
      td.label { color: #9CA3AF; width: 40%; }
      .amount-box { background: #F6F7F5; border-radius: 10px; padding: 16px; margin: 18px 0; text-align: center; }
      .amount-box .l { font-size: 11px; color: #9CA3AF; text-transform: uppercase; letter-spacing: 1px; }
      .amount-box .v { font-size: 26px; font-weight: 800; color: ${isPaidDoc ? "#2A9D8F" : "#B0362A"}; margin-top: 4px; }
      .status-pill { display:inline-block; padding: 3px 10px; border-radius: 999px; font-size: 11px; font-weight: 600; background:${pillBg}; color:${pillColor}; margin-top: 6px; }
      .foot { margin-top: 30px; font-size: 11px; color: #9CA3AF; text-align: center; border-top: 1px dashed #D1D5DB; padding-top: 12px; }
      @media print { body { padding: 10px; } }
    </style>
    </head><body>
      <div class="head">
        <div>
          <div class="brand">PT. DATA BUANA NUSANTARA</div>
          <div class="sub">${docLabel}${isPaidDoc ? "" : " — Mohon segera diselesaikan"}</div>
        </div>
        <div class="inv-title">
          <div class="t">NO. INVOICE</div>
          <div class="n">${noInvoice}</div>
        </div>
      </div>
      <table>
        <tr><td class="label">Nama Pelanggan</td><td>${customer.nama || "-"}</td></tr>
        <tr><td class="label">Daerah</td><td>${customer.daerah || "-"}</td></tr>
        ${customer.telepon ? `<tr><td class="label">Telepon</td><td>${customer.telepon}</td></tr>` : ""}
        <tr><td class="label">Periode Tagihan</td><td>${monthLabel(payment.month)}</td></tr>
        <tr><td class="label">${dateLabel}</td><td>${tanggalIndo(payment.tanggal)}</td></tr>
        <tr><td class="label">Ditagih Oleh</td><td>${penagihNama || "-"}</td></tr>
        ${payment.keterangan ? `<tr><td class="label">Keterangan</td><td>${payment.keterangan}</td></tr>` : ""}
      </table>
      <div class="amount-box">
        <div class="l">${amountLabel}</div>
        <div class="v">${rupiah(payment.jumlah)}</div>
        <div class="status-pill">${statusLabel}</div>
      </div>
      <div class="foot">${isPaidDoc ? "Invoice ini dibuat otomatis oleh sistem sebagai bukti pembayaran yang sah." : "Dokumen ini adalah tagihan, mohon lakukan pembayaran kepada petugas penagih."}</div>
    </body></html>
  `);
  w.document.close();
  w.focus();
...
// Minta jumlah tagihan lewat prompt lalu langsung cetak dokumen "Tagihan" (untuk pelanggan yang belum bayar tapi minta invoice duluan).
function promptTagihan(customer, month, penagihNama) {
  const raw = window.prompt(`Jumlah tagihan untuk ${customer.nama} (Rp):`, "");
  if (raw === null) return;
  const jumlah = Number(String(raw).replace(/[^0-9]/g, "")) || 0;
  if (!jumlah) { alert("Jumlah tagihan tidak valid."); return; }
  printInvoice(customer, { month, status: "tagihan", jumlah, tanggal: new Date().toISOString(), keterangan: "" }, penagihNama);
}
...
const STATUS_LABEL = {
  ...
  lunas_dimuka: { label: "Lunas · Bayar 2 Bulan Dimuka", color: "#7C3AED", bg: "#F3E8FF" },
  lunas_dimuka_otomatis: { label: "Lunas · Sudah Dibayar Dimuka Bulan Lalu", color: "#7C3AED", bg: "#F3E8FF" },
...
// Status yang dianggap "lunas/berhasil ditarik" — dipakai untuk total pendapatan, dst.
const isLunasStatus = (status) => status === "cash" || status === "transfer" || status === "lunas_dobel" || status === "lunas_dimuka" || status === "lunas_dimuka_otomatis";
...
async function savePaymentRecord({ month, customer, status, keterangan, jumlah, penagihUid, dobel }) {
  ...
  const isDimuka = status === "lunas_dimuka";
  ...
  await setDoc(doc(db, "payments", payId), {
    ...
    dobel: isDobel || isDimuka,
  });
  // Efek "dobel bulan depan" (tunggakan bulan lalu dibayar sekaligus bulan ini)
  ...
  // Efek "bayar dimuka 2 bulan sekaligus": bulan ini bayar untuk bulan ini + bulan depan.
  // Uang sudah tercatat penuh di bulan ini, jadi catatan bulan depan dibuat otomatis
  // dengan status "sudah bayar" tapi jumlah 0, supaya penagih bulan depan tidak perlu
  // menagih ulang dan tidak dihitung dobel.
  if (isDimuka) {
    const nMonth = nextMonthKey(month);
    await setDoc(doc(db, "payments", `${nMonth}_${customer.id}`), {
      month: nMonth, customerId: customer.id, status: "lunas_dimuka_otomatis",
      keterangan: `Sudah dibayar dimuka pada ${monthLabel(month)}`,
      jumlah: 0, penagihId: penagihUid, tanggal: new Date().toISOString(),
      dobel: false,
    });
  }
}
...
function AdminView({ profile, customers, penagihList, onLogout }) {
  ...
  const totalDimuka = payments.filter((p) => p.status === "lunas_dimuka").reduce((s, p) => s + p.jumlah, 0);
    ...
    <div className="min-h-screen pb-6" style={{ background: CREAM }}>
      <div className="px-5 pt-5 pb-4 sticky top-0 z-10 relative overflow-hidden" style={{ background: NAVY }}>
        ...
        <div className="flex gap-2 mt-4 overflow-x-auto">
          {[["ringkasan","Ringkasan"],["pelanggan","Pelanggan"],["penagih","Kinerja"],["riwayat","Riwayat Bulan"]].map(([k,label]) => (
        ...
        {tab === "ringkasan" && (
          <>
            <div className="flex gap-3 flex-wrap mb-3">
              ...
              <StatCard icon={Wallet} label={`Pendapatan ${monthLabel(viewMonth)}`} value={rupiah(totalCash + totalTransfer + totalDobel + totalDimuka)} accent={TEAL} sub={`${lunas.length} pelanggan lunas`} />
            ...
            <div className="flex gap-3 flex-wrap mb-3">
              ...
              <StatCard icon={Wallet} label="Total Bayar Dimuka" value={rupiah(totalDimuka)} accent="#7C3AED" sub={`${payments.filter((p) => p.status === "lunas_dimuka").length} pelanggan`} />
            </div>
        ...
        {tab === "riwayat" && (
          <>
              ...
              {lunasList.map((p) => (
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-400 mb-1">{p.status === "cash" ? "Cash" : p.status === "transfer" ? "Transfer" : "Bayar Dobel"}</div>
                    {p.customer && (
                      <button onClick={() => printInvoice(p.customer, p, penagihList.find((pg) => pg.uid === p.penagihId)?.nama)}
                        className="flex items-center gap-1 text-xs font-medium ml-auto" style={{ color: NAVY }}>
            ...
            <div className="rounded-2xl bg-white border border-gray-100 p-4 mb-3">
              ...
              <div className="space-y-2">
                {belumBayarTanpaKet.map((c) => <PayRow key={c.id} customer={c} existing={paidMap.get(c.id)} onSave={saveRiwayat} penagihNama={penagihList.find((pg) => pg.uid === c.penagihId)?.nama} month={viewMonth} />)}
                {belumBayarTanpaKet.length === 0 && <p className="text-xs text-gray-400">Semua pelanggan yang belum bayar sudah punya keterangan.</p>}
              </div>
            </div>
...
// ---------- Panel Penagih ----------
function PayRow({ customer, existing, onSave, penagihNama, month }) {
  ...
  const needsJumlah = status === "cash" || status === "transfer" || status === "kurang" || status === "lunas_dobel" || status === "lunas_dimuka";
      ...
      <div className="rounded-xl bg-white border border-gray-100 p-3" style={{ borderLeft: `4px solid ${st?.color || AMBER}` }}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {isPaid && (
              <button onClick={() => printInvoice(customer, existing, penagihNama)} className="flex items-center gap-1 text-xs font-medium" style={{ color: NAVY }}>
                <FileText size={12} /> Invoice
              </button>
            )}
            {!isPaid && (
              <button onClick={() => promptTagihan(customer, existing.month, penagihNama)} className="flex items-center gap-1 text-xs font-medium" style={{ color: "#B0362A" }}>
  ...
  return (
    <div className="rounded-xl bg-white border border-gray-100 p-3" style={{ borderLeft: `4px solid ${existing ? (STATUS_LABEL[existing.status]?.color || TEAL) : "#D1D5DB"}` }}>
      ...
      <div className="text-xs text-gray-400 mb-2 flex items-center gap-2 flex-wrap">
        ...
        <button onClick={() => promptTagihan(customer, month || existing?.month, penagihNama)} className="flex items-center gap-1 text-xs font-medium" style={{ color: "#B0362A" }}>
          <FileText size={12} /> Buat Tagihan
        </button>
      </div>
      <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mb-2 outline-none">
        ...
        <option value="lunas_dimuka">Lunas · Bayar 2 Bulan Sekaligus (Bulan Ini + Bulan Depan)</option>
        {existing?.status === "lunas_dimuka_otomatis" && <option value="lunas_dimuka_otomatis">Lunas · Sudah Dibayar Dimuka (Otomatis)</option>}
      ...
      {status === "lunas_dobel" && (
        <p className="text-xs mb-2 p-2 rounded-lg" style={{ background: "#FBEAE6", color: "#B0362A" }}>
          Isi jumlah total yang dibayar (bulan ini + tunggakan bulan lalu). Tunggakan dianggap lunas.
        </p>
      )}
      {status === "lunas_dimuka" && (
        <p className="text-xs mb-2 p-2 rounded-lg" style={{ background: "#F3E8FF", color: "#7C3AED" }}>
          Isi jumlah total yang dibayar (bulan ini + bulan depan sekaligus, dibayar dimuka). Bulan depan pelanggan ini otomatis tercatat "sudah bayar" tanpa perlu ditagih ulang.
        </p>
      )}
      {needsJumlah && (
...
function PenagihView({ profile, uid, customers, onLogout }) {
  ...
  const filtered = mine
    ...
    .filter((c) => {
      ...
      if (bayarFilter === "dimuka") return paidMap.get(c.id)?.status === "lunas_dimuka" || paidMap.get(c.id)?.status === "lunas_dimuka_otomatis";
      return true;
    })
    .sort((a, b) => a.nama.localeCompare(b.nama, "id", { sensitivity: "base" }));
  ...
  const bayarDimukaCount = mine.filter((c) => paidMap.get(c.id)?.status === "lunas_dimuka").length;
  ...
  const mineDimuka = [...paidMap.values()].filter((p) => p.status === "lunas_dimuka").reduce((s, p) => s + p.jumlah, 0);
        ...
        {showDetail && (
          <div className="mt-2 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="rounded-2xl bg-white/10 p-3"><div className="text-white/70 text-xs mb-1">Bayar Dimuka</div><div className="text-white font-mono font-semibold text-sm">{rupiah(mineDimuka)}</div></div>
            </div>
          </div>
        )}
      </div>
      <div className="p-5">
        ...
        <div className="flex gap-2 mb-3 overflow-x-auto">
          {[
            ...
            ["dimuka", `Sudah Bayar Dimuka (${bayarDimukaCount})`, "#7C3AED"],
          ].map(([k, label, color]) => (
        ...
        <div className="space-y-2">
          {filtered.map((c) => <PayRow key={c.id} customer={c} existing={paidMap.get(c.id)} onSave={save} penagihNama={profile.nama} month={entryMonth} />)}
